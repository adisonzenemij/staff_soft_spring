...
public interface TgRoleDataConjunct extends ListCrudRepository<TgRoleDataEntity, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT COUNT(*) FROM tg_role_data")
    int countAll();

    // Obtener el conteo de registros de la tabla
    @Query(nativeQuery = true, value = "SELECT * FROM tg_role_data WHERE id_register = :idRegister")
    List<TgRoleDataEntity> findIdRegister(@Param("idRegister") String idRegister);

    @Query(nativeQuery = true, value = "UPDATE tg_role_data SET cd_name = :cdName WHERE id_register = :idRegister")
    void updateData(@Param("idRegister") int idRegister, @Param("cdName") String cdName);
